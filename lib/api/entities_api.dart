...
class EntitiesApi {
  EntitiesApi({required this.baseUrl});
  ...
  Future<List<Map<String, dynamic>>> fetchEntities({
    ...
    double radiusKm = 50,
    int limit = 50,
    bool serverSideGeo = false, // kept for compatibility; backend can ignore it
  }) async {
    final uri = Uri.parse('$baseUrl/entities').replace(
      queryParameters: {
        'lat': lat.toString(),
        'lon': lon.toString(),
        'radiusKm': radiusKm.toString(),
        'limit': limit.toString(),
        'serverSideGeo': serverSideGeo.toString(),
      },
    );
    ...
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('fetchEntities failed (${res.statusCode}): ${res.body}');
    }
    ...
    // fallback if backend returns a raw array
    if (decoded is List) {
      return decoded
          .whereType<Map>()
          .map((e) => e.map((k, v) => MapEntry(k.toString(), v)))
  ...
  Future<Map<String, dynamic>> createEntity({
    ...
    String address = '',
    String contactPhone = '',
    Map<String, dynamic>? location, // {"lat":..,"lon":..}
    bool remote = false,
  }) async {
    final uri = Uri.parse('$baseUrl/entities');
    ...
    if (res.statusCode < 200 || res.statusCode >= 300) {
      throw Exception('createEntity failed (${res.statusCode}): ${res.body}');
    }
